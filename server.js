...
const { getUser, getLinkedAccounts, linkAccount, addManualContribution, getManualContributions, getAllUsersWithLinkedAccounts, deleteUser, updateUserStatus } = require('./src/db/database');
const { requireAdmin, ADMIN_PASSWORD } = require('./src/middleware/auth');
...
// Admin: Toggle Status (Protected)
app.patch('/api/admin/user/:id/status', requireAdmin, async (req, res) => {
    try {
        const userId = req.params.id;
